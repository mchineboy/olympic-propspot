<script lang="ts">
    ...
    const categories = ['Clothing', 'Furniture', 'Accessories', 'Electronics', 'Wigs', 'Other'] as const;
    ...
    $: availableTypes = editingProp.category && editingProp.category in types 
      ...
      if (target.files && target.files[0]) {
        ...
        reader.onload = (e) => {
          if (typeof e.target?.result === 'string') {
            editingProp.imagePreviewUrl = e.target.result;
          }
        };
        reader.onerror = () => {
  ...
  <form on:submit|preventDefault={saveProp} class="space-y-4">
    ...
    {#if editingProp.imagePreviewUrl || editingProp.imageUrl}
      <div class="mt-2">
        <img src={editingProp.imagePreviewUrl || editingProp.imageUrl} alt="Prop preview" class="h-auto max-w-full rounded-lg shadow-md">
      </div>
    {/if}
  
    <div class="flex justify-between pt-4">
      <button type="submit" class="px-4 py-2 font-bold text-white bg-purple-600 rounded hover:bg-purple-700" disabled={isLoading}>
        {isLoading ? 'Saving...' : 'Save'}
      </button>
      <button type="button" on:click={cancel} class="px-4 py-2 font-bold text-gray-800 bg-gray-300 rounded hover:bg-gray-400" disabled={isLoading}>
        Cancel
      </button>
    </div>
  </form>