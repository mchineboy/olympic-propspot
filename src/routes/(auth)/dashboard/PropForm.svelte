<script lang="ts">
		...
		if (target.files && target.files[0]) {
			...
			reader.onload = (e) => {
				if (typeof e.target?.result === 'string') {
					// Only use this for preview, not for storage
	...
	async function saveProp() {
		let imageUrl: string | undefined | null = editingProp.imageUrl; // Keep the existing URL if no new image is uploaded
		...
		const propToSave = {
			...editingProp,
			imageUrl: imageUrl // This will be the Storage URL, not the base64 data
		};
...
<form on:submit|preventDefault={saveProp} class="space-y-4">
	...
	<div>
		<label for="prop-image" class="block text-sm font-medium text-purple-700"
			>Upload Photo or Take Picture</label
		>
		<input
			type="file"
			id="prop-image"
	...
	{#if editingProp.imagePreviewUrl || editingProp.imageUrl}
		<div class="mt-2">
			<img
				src={editingProp.imagePreviewUrl || editingProp.imageUrl}
				alt="Prop preview"
