<script lang="ts">
  ...
  let editingProp: Prop;

  $: {
    editingProp = prop ? convertOldPropFormat({ ...prop }) : getEmptyProp();
  }
  ...
  function handleImagesChange(event: CustomEvent<string[]>) {
    editingProp.imageUrls = event.detail;
    editingProp = { ...editingProp };
  }
  ...
  function handleAttributesChange(event: CustomEvent<PropAttribute[]>) {
    editingProp.attributes = event.detail;
    editingProp = { ...editingProp };
  }
...
<form on:submit|preventDefault={saveProp} class="space-y-4">
  ...
  <div>
    <label id="prop-images-label" class="block mb-2 text-sm font-medium text-purple-700">Images</label>
    <MobileImageUpload
